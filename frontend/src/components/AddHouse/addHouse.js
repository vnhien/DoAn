...
export default class AddHouse extends React.Component {
    ...
    handleSubmit = async (e) => {
        ...
        if (data.province === '') {
            ...
            try {
                var response = await api.createhouse(formData);
    ...
    render = () => {
        if (this.props.user === null || this.props.user.acctype !== 'Chủ nhà trọ') {
            return <Redirect to='/' />
        }
        ...
        if (this.state.createState === 'success') {
            return <Redirect to={{ pathname: "/listhouse", state: { from: 'addhouse' } }} />
        }
            ...
            <div className="content-container container-fluid">
                <div className="create-form-container">
                    <div className="navigator row">
                        <a href="/">HOME &nbsp;</a>
                        <p><i className="fas fa-angle-double-right" style={{ color: '#2196F3' }}></i>&nbsp; Thêm nhà trọ mới</p>
                    </div>
                    ...
                    <form className="create-form" id="create-form" method="post" acceptCharset="UTF-8" encType="multipart/form-data"
                        ...
                        <div className="row">
                            ...
                            <div className="col-5" style={{ marginRight: "auto" }}>
                                <input type="text" onChange={e => { this.changeHandler(e) }} className="form-control" required="required" name="name" placeholder="Nhập tên nhà trọ mới" />
                            </div>
                        </div>
                        ...
                        <div className="row">
                            ...
                            <div className="col-5" style={{ marginRight: "auto" }}>
                                <input type="text" onChange={e => { this.changeHandler(e) }} className="form-control" required="required" name="address" placeholder="Địa chỉ phòng cho thuê" />
                            </div>
                        </div>
                        ...
                        <div className="row">
                            ...
                            <div className="col-5" style={{ marginRight: "auto" }}>
                                <textarea onChange={e => { this.changeHandler(e) }} style={{ height: "240px" }} name="description" className="form-control" required="required" placeholder="Thông tin mô tả thêm về nhà trọ" />
                            </div>
                        </div>
                        <div className="row imageInput-container">
                            ...
                            <div className="col-5 imageInput" style={{ marginRight: "auto" }}>
                                <input onChange={this.changeImage} type="file" name="image" id="image" required="required" />
    ...
    componentDidMount = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                this.setState({ 'provinceSelect': { isLoading: false, data: response.data } })
            } else {
                this.setState({ 'serverError': true })
            }
        } catch (err) {
    ...
    changeImage = (event) => {
        ...
        if (event.target.files && event.target.files[0]) {
            ...
            temp[fieldName] = '';
            this.setState({ formData: temp, previewImage: '' });
        }
