...
exports.getNumberUser = async (req, res) => {
    ...
    try{
        var result = await Account.getNumberUser();
...
exports.getStatisticUser = async (req, res) => {
    ...
    try{
        var result = await Account.getStatisticUser();
        res.status(200).json(result)
    }catch(err){
        res.status(500).end();
        console.log(err)
...
exports.Register = function (req, res) {
    ...
    Account.createAccount(data).then((result) => {
        if (result.ok === 1) {
            res.status(200).send({ "Successed": true })
        } else {
