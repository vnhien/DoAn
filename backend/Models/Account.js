...
//Đếm số lượng user
exports.getNumberUser = function () {
  return knex("account").count("userId")
  .then((res)=>{
    return { 
      "ok": 1 ,
      "numberuser": res
...
exports.getStatisticUser = function () {
  return knex("account").select('date').count("userId",{as:"numberuser"}).groupByRaw("month(date)").orderByRaw("month(date)")
  .then((res)=>{
     res.map(a => {
